...
degrees from it), so that adjacent vertices' beam lines coincide. The
...
- **THEN** each horizontal beam's two end faces are each within 0.05 mm of the beam line reference of its two vertices, and the perpendicular distance from the centre to every beam axis equals `tower_radius / 2 + 3.75 + 16` within 0.05 mm
...
- **WHEN** the frame is built with the beam, rail, tab and glass screws declared as press supports
- **THEN** every rigid solid of the frame is supported under gravity
